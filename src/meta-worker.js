...
class MetaWorker {
  ...
  async submitPrompt(prompt, options = {}) {
    ...
    // Schedule run after the current chain (run executes only once).
    const runTask = this.submitChain.then(run, run);
    // Update chain so the next request waits for this one.
    this.submitChain = runTask.then(() => undefined, () => undefined);
    // Race the run against a queue-wait timeout.
  ...
  async _submitPromptCore(prompt, { forceNewChat = false, sessionId = null, sessionUrl = null, timeoutMs = DEFAULT_RESPONSE_TIMEOUT_MS, alwaysFilePrompt = false, cancelRef = null } = {}) {
    ...
    try {
      ...
      const submittedPromptText = useFilePrompting
        ? "INSTRUCTIONS: Read the attached file. It contains your role, tools, and the task. Your ENTIRE reply must be <tool_call> XML blocks — nothing else. No planning, no describing, no greeting, no canvas, no preview links. Do NOT use Meta AI website builder, canvas, or any internal tool. Do NOT offer download links. The ONLY valid output is <tool_call> XML. First character of your reply must be '<'."
        : promptText;
